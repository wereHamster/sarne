...
async fn run_payment_probe(app: &mut App, payment_probe: &mut PaymentProbe) -> Result<()> {
    ...
    for attempt in 0..3 {
        ...
        match res.failure {
            ...
            Some(failure) => {
                // Failure on source index zero (0) indicates a failure on the local node.
                // We should that not attribute to any other node (it's our own problem).
                //
                ...
                // If the failure was generated by the last hop, it means we've reached it
                // and consider the probe a success.
                //
                // Store the probe attempt and return;
                if failure.failure_source_index as usize == route.hops.len() {
                    payment_probe.attempts.push(PaymentProbeAttempt {
                        seqno: attempt,
                ...
                // The probe failed, Store a failed attempt and continue probing through
                // other routes.
                payment_probe.attempts.push(PaymentProbeAttempt {
                    seqno: attempt,
